...
class SiteSettingsForm(FlaskForm):
    events_confirmed = BooleanField("Events Confirmed")

    ticket_url = StringField("Ticket URL", validators=[Optional(), URL(message="Enter a valid URL")])

    email = StringField("Email", validators=[Optional()])
    phone = StringField("Phone", validators=[Optional()])
    instagram_url = StringField("Instagram URL", validators=[Optional(), URL(message="Enter a valid URL")])
    ...
    founder1_name = StringField("Founder 1 Name", validators=[Optional(), Length(max=200)])
    founder1_image = StringField("Founder 1 Image URL", validators=[Optional(), Length(max=500)])

    founder2_name = StringField("Founder 2 Name", validators=[Optional(), Length(max=200)])
    founder2_image = StringField("Founder 2 Image URL", validators=[Optional(), Length(max=500)])
